...
- 纯本地运行，**字体文件全程不出本机**
- 支持三种分片模式（站点 / 混合 / 字频）与手动微调（合并 / 拆分 / 钉字 / 排除）
...
- **智能建议 / 校验提示**：基于字体与参数给出的可读建议（如建议改用 woff2、参数合理性等）。
- **分片清单**：
  - 每片显示序号 `#`、字数、覆盖的**字符样本**（鼠标悬停查看精确 `unicode-range`）、文件大小；
  - 高亮行 = 样本文本会命中的片；
  - 可在每片行内「合并↓ / 拆分」，底部「钉字 / 排除 / 重置手动编辑」（详见第 8 节）。
- **产物预览**：可直接复制的 `@font-face` CSS（见第 6 节部署）。
